...
namespace JumpProject
{
    /*
    초성 : 음절의 구성에서 처음소리인 자임. ('님' 에서 'ㄴ')
    중성 : 음절의 구성에서 중간소리인 모음. ('땅' 에서 'ㅏ', '들' 에서 'ㅡ')
    종성 : 음절의 구성에서 마지막소리인 자음('감' 에서 'ㅁ', '공' 에서 'ㅇ')
    자음 : ㄱㄴㄷㄹ
    모음 : ㅑㅐㅏㅏㅏㅏ
    */
    ...
    class AutomataKey
    {
        ...
        public int m_nStatus;		// 단어조합상태
        public int[] m_nPhonemez = new int[5]; // 음소[초,중,종,곁자음1,곁자음2]
        public char m_completeWord;    // 완성글자

        public AutomataKey() // 생성자
        {
            Clear();
        }
        ...
        // ***********포인터 이슈있음**************
        public char SetKeyCode(int nKeyCode, int tKeycode)
        {
            m_completeWord = '\0';
            ...
            switch (tKeycode)
            {
                ...
                case 1:
                    ...
                    switch (m_nStatus)
                    {
                        case (int)HAN_STATUS.HS_FIRST:
                            m_nPhonemez[0] = nKeyCode;
                            ingWord = eng_TABLE[m_nPhonemez[0]];
                            m_nStatus = (int)HAN_STATUS.HS_FIRST_V;
                            break;


                        case (int)HAN_STATUS.HS_FIRST_V:
                            if (!eng_MixInitial(nKeyCode))
                            {
                                //if (!pre_trance) // 이전에 영어
                                //{
                                //    m_completeWord = ENG_TABLE[m_nPhonemez[0]];
                                //}
                                //else //이전에 한글
                                //{
                                //    m_completeWord = SOUND_TABLE[m_nPhonemez[0]];
        ...
        private bool eng_MixInitial(int nKeyCode) // 영소 완성
        {
            int i = 0;
            do
            {
                if (MIXED_ENG_CONSON[i, 0] == m_nPhonemez[0] && MIXED_ENG_CONSON[i, 1] == nKeyCode)
                {
                    m_nPhonemez[0] = MIXED_ENG_CONSON[i, 2];
                    return true;
                }
            }
            while (++i < 32);
        ...
        private bool Eng_MixInitial(int nKeyCode) // 영대 완성
        {
            int i = 0;
            do
            {
        ...
        private void e_CombineIngWord(int status)
        {
            switch (m_nStatus)
            {
                case (int)HAN_STATUS.HS_FIRST:
        ...
        //영어 합성 테이블
        int[,] MIXED_ENG_CONSON = new int[32, 3]
        {
            {0,0,1}, // a,a,b
	        {1,0,2}, // b,a,c
	        {2,0,0}, // c,a,a
        ...
        // 초성 합성 테이블
        int[,] MIXED_CHO_CONSON = new int[19, 3]
        {
            { 0, 0,15 }, // ㄱ,ㄱ,ㅋ
	        {15, 0, 1}, // ㅋ,ㄱ,ㄲ
	        { 1, 0, 0}, // ㄲ,ㄱ,ㄱ
        ...
        // 종성 합성 테이블
        int[,] MIXED_JONG_CONSON = new int[27, 3]
        {
            {41,41,64}, // ㄱ,ㄱ,ㅋ
	        {64,41,42}, // ㅋ,ㄱ,ㄲ
	        {42,41,41}, // ㄲ,ㄱ,ㄱ
...
/*참고 및 출처
https://blog.naver.com/fanta2001/220159140421
http://ehpub.co.kr/1-%ED%95%9C%EA%B8%80-%EB%AC%B8%EC%9E%90-%EC%83%9D%EC%84%B1%EA%B8%B0/ 
*/
